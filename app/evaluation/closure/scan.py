"""M5.6 输入/报告共用敏感扫描：发现 query、正文、密钥立即失败。"""
...
def scan_closure_payload(payload: object, path: str = "payload") -> None:
    ...
    if isinstance(payload, dict):
        for key, value in payload.items():
            ...
            scan_closure_payload(value, f"{path}.{key}")
    elif isinstance(payload, list):
        for index, value in enumerate(payload):
            scan_closure_payload(value, f"{path}[{index}]")
